...
const schema = z.object({
  ...
  personality: z.string().min(10, "Personality must be at least 10 characters").max(1000),
  backstory: z.string().max(2000).optional(),
  greeting: z.string().min(5, "Greeting must be at least 5 characters").max(500),
  category: z.string(),
  isPublic: z.boolean(),
  isNSFW: z.boolean(),
})
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Design your AI persona with a unique personality
        </p>
      </div>
        ...
        <Card className="p-4 space-y-4">
          ...
          <div>
            ...
            <Input
              id="name"
              {...register("name")}
              placeholder="e.g. Aria, Shadow Knight, Professor Smith"
              className="mt-1"
            />
          ...
          <div>
            ...
            <Textarea
              id="greeting"
              {...register("greeting")}
              placeholder="The first message this character sends to users..."
              className="mt-1 h-20 resize-none"
            />
            {errors.greeting && (
              <p className="text-destructive text-xs mt-1">{errors.greeting.message}</p>
            )}
          </div>
        </Card>
        ...
        <Card className="p-4 space-y-4">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Describe personality traits, speaking style, quirks, values..."
              className="mt-1 h-28 resize-none"
            />
            {errors.personality && (
              <p className="text-destructive text-xs mt-1">{errors.personality.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            <Textarea
              id="backstory"
              {...register("backstory")}
              placeholder="Character's history, origin story, past experiences..."
              className="mt-1 h-28 resize-none"
            />
          </div>
        </Card>
        ...
        <Card className="p-4 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                This character contains mature content (18+)
              </p>
            </div>
            <Switch
              checked={isNSFW}
              onCheckedChange={(val) => setValue("isNSFW", val)}
            />
          </div>
        </Card>
